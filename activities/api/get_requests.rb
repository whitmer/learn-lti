get = Activity.add(:get_requests, :api)
  ...
  get.api_test :error_handling, :answer => "You are not authorized to perform that action.", :explanation => <<-EOF
    ...
    browser's debugger: <code>$.getJSON("/api/v1/users/self/profilex").fail(function(xhr) { console.log(JSON.parse(xhr.responseText.replace(/while\(1\);/, ''))); })</code>.
    ...
    code: <code>$.getJSON("/api/v1/users/230519/profile").fail(function(xhr) { console.log(JSON.parse(xhr.responseText.replace(/while\(1\);/, ''))); })</code>.
    ...
    <code>$.getJSON("/api/v1/users/0/profile").fail(function(xhr) { console.log(JSON.parse(xhr.responseText.replace(/while\(1\);/, ''))); })</code>.
    ...
    <code>$.getJSON("/api/v1/users/self/profile?access_token=asdf").fail(function(xhr) { console.log(JSON.parse(xhr.responseText.replace(/while\(1\);/, ''))); })</code>.
    ...
    <code>$.getJSON("/api/v1/courses/123/external_feeds").fail(function(xhr) { console.log(JSON.parse(xhr.responseText.replace(/while\(1\);/, ''))); })</code> 
