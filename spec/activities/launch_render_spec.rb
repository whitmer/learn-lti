...
describe 'Activity Rendering' do
  ...
  it "should require launch url for post tests" do
    get "/fake_launch"
    post "/test/post_launch/1", {}, 'rack.session' => {"farthest_for_post_launch" => 1}
    ...
    describe "#{activity.id} render checks" do  
      activity.tests.each_with_index do |test, idx|
        it "should allow checking for #{test[:args][:param]}" do
          get "/fake_launch"
          get "/launch/#{activity.id}/#{idx}", {}, 'rack.session' => {"farthest_for_#{activity.id}" => activity.tests.length}
        ...
        if test[:type] == :fill_in || test[:type] == :redirect || test[:type] == :grade_passback
          it "should allow posting to launch for #{test[:args][:param]}" do
            get "/fake_launch"
            post "/test/#{activity.id}/#{idx}", {'launch_url' => 'http://www.example.com/launch/it/up'}, 'rack.session' => {"farthest_for_#{activity.id}" => activity.tests.length}
