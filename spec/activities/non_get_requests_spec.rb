...
describe 'Non-get_with_session Request Tests' do
  ...
  it "should succeed when correct folder information found" do
    ...
    get_with_session "/launch/non_get/0", {}
    answer = 1236.to_s
    Api.any_instance.should_receive(:get).with('/api/v1/users/self/folders/root').and_return({'id' => 1234})
    Api.any_instance.should_receive(:get).with('/api/v1/folders/1234/folders').and_return(ArrayWithPagination.new [{}, {}, {'id' => answer, 'name' => 'grindylow'}])
    post_with_session "/validate/non_get/0", {'answer' => answer}
  ...
  it "should fail when folder information not found" do
    ...
    answer = 1236.to_s
    Api.any_instance.should_receive(:get).with('/api/v1/users/self/folders/root').and_return({'id' => 1234})
    Api.any_instance.should_receive(:get).with('/api/v1/folders/1234/folders').and_return(ArrayWithPagination.new [{}, {}, {'id' => answer}])
    post_with_session "/validate/non_get/0", {'answer' => answer}
    json = JSON.parse(last_response.body)
  ...
  it "should fail when folder information doesn't match answer" do
    ...
    Api.any_instance.should_receive(:get).with('/api/v1/users/self/folders/root').and_return({'id' => 1234})
    Api.any_instance.should_receive(:get).with('/api/v1/folders/1234/folders').and_return(ArrayWithPagination.new [{}, {}, {'id' => answer, 'name' => 'grindylow'}])
    post_with_session "/validate/non_get/0", {'answer' => "1237"}
    json = JSON.parse(last_response.body)
    json['answer'].should == answer
  ...
  it "should fail when short name not entered correctly" do
    ...
    answer = "bob_jones"
    Api.any_instance.should_receive(:get).with('/api/v1/users/self/profile').and_return({'short_name' => answer})
    post_with_session "/validate/non_get/1", {'answer' => answer + "s"}
    json = JSON.parse(last_response.body)
    json['answer'].should == answer
    json['correct'].should == false
  end
  ...
  it "should succeed when event correctly deleted" do
    ...
    post_with_session "/validate/non_get/2", {'answer' => answer}
    json = JSON.parse(last_response.body)
    json['answer'].should == answer
    json['correct'].should == true
  end
