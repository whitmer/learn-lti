...
describe 'Content Extensions Activity' do
  ...
  it "should succeed with the correct parameters" do
    ...
    get_with_session "/launch/content_test/4", {}
    post_with_session "/test/content_test/4", {'launch_url' => 'http://www.example.com/launch'}
  ...
  it "should fail with incorrect parameters" do
    ...
    get_with_session "/tool_return/content_test/4/#{rand_id}?embed_type=files&url=http%3A%2F%2Fwww.bacon.com%2Fbacon.docx&content_type=application%2Fvnd.openxmlformats-officedocument.wordprocessingml.document&friends=2&alt=hope"
    html = Nokogiri::HTML(last_response.body)
    json = JSON.parse(html.css('#result_data').text)
  ...
  it "should fail with no parameters" do
    ...
    get_with_session "/tool_return/content_test/4/#{rand_id}"
    html = Nokogiri::HTML(last_response.body)
    json = JSON.parse(html.css('#result_data').text)
    json['correct'].should == false
  end
