...
describe 'API GET Requests' do
  ...
  it "should allow answering a :status_code question" do
    ...
    get "/launch/get_requests/0", {}
    answer = Digest::MD5.hexdigest(Date.today.iso8601)[1, 15]
    post "/validate/get_requests/0", {'answer' => answer}
    json = JSON.parse(last_response.body)
    json['answer'].should == answer
    json['correct'].should == true
    json['next'].should == "/launch/get_requests/1"

    post "/validate/get_requests/0", {'answer' => answer + "a"}
    json = JSON.parse(last_response.body)
    json['answer'].should == answer
    json['correct'].should == false
  end
  
  it "should get the correct id from a profile api call" do
    fake_launch({'farthest_for_get_requests' => 10})
    get "/launch/get_requests/1", {}
    answer = 'asdf'
    Api.any_instance.should_receive(:get).and_return({'id' => answer})
    post "/validate/get_requests/1", {'answer' => answer}
    json = JSON.parse(last_response.body)
    json['answer'].should == answer
    json['correct'].should == true
  ...
  it "should error on different id from a profile api call" do
    ...
    answer = 'asdf'
    Api.any_instance.should_receive(:get).and_return({'id' => answer})
    post "/validate/get_requests/1", {'answer' => "You are not authorized to perform that action."}
    json = JSON.parse(last_response.body)
    json['answer'].should == answer
    json['correct'].should == false
  end
  ...
  it "should ask for the correct error message on unauthorized api call" do
    ...
    post "/validate/get_requests/2", {'answer' => answer}
    json = JSON.parse(last_response.body)
    json['answer'].should == answer
    json['correct'].should == true
  end
  ...
  it "should ask for https on the https question" do
    fake_launch({'farthest_for_get_requests' => 10})
    answer = "https"
    post "/validate/get_requests/3", {'answer' => answer}
    json = JSON.parse(last_response.body)
    json['answer'].should == answer
    json['correct'].should == true
  ...
  it "should get the correct calendar -> ics value from a profile api call" do
    fake_launch({'farthest_for_get_requests' => 10})
    get "/launch/get_requests/4", {}
    answer = 'a8toyn24ty2m8ty2'
    Api.any_instance.should_receive(:get).and_return({'calendar' => {'ics' => answer}})
    post "/validate/get_requests/4", {'answer' => answer}
    json = JSON.parse(last_response.body)
    json['answer'].should == answer
    json['correct'].should == true
  end
  
  it "should get the correct id of the first course in a courses api call" do
    fake_launch({'farthest_for_get_requests' => 10})
    get "/launch/get_requests/5", {}
    answer = 'a8toyn24ty2m8ty2'
    Api.any_instance.should_receive(:get).and_return([{'id' => answer}])
    post "/validate/get_requests/5", {'answer' => answer}
    json = JSON.parse(last_response.body)
    json['answer'].should == answer
    json['correct'].should == true
  end
  ...
  context "filtering" do
    ...
    it "should error on empty results during lookup" do
      ...
      Api.any_instance.should_receive(:get).with('/api/v1/users/self/profile').and_return({'id' => 1})
      Api.any_instance.should_receive(:get).with("/api/v1/calendar_events?context_codes[]=user_1&type=event&start_date=2010-12-31&end_date=2011-01-02").and_return([])
      post "/validate/get_requests/6", {'answer' => answer}
      json = JSON.parse(last_response.body)
  ...
  it "should get the correct term id when included on courses favorites call" do
    fake_launch({'farthest_for_get_requests' => 10})
    get "/launch/get_requests/7", {}
    answer = 123
    Api.any_instance.should_receive(:get).with('/api/v1/users/self/favorites/courses?include[]=term').and_return([{'term' => {'id' => answer}}])
    post "/validate/get_requests/7", {'answer' => answer}
    json = JSON.parse(last_response.body)
    json['answer'].should == answer
    json['correct'].should == true      
  end
  
  it "should get the correct id of the first communication channel" do
    fake_launch({'farthest_for_get_requests' => 10})
    get "/launch/get_requests/8", {}
    answer = 1236
    Api.any_instance.should_receive(:get).with('/api/v1/users/self/communication_channels').and_return([{'id' => answer}])
    post "/validate/get_requests/8", {'answer' => answer}
    json = JSON.parse(last_response.body)
    json['answer'].should == answer
    json['correct'].should == true      
  end
