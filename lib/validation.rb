module Sinatra
  module Validation
    def self.registered(app)
      ...
      app.get '/tool_return/:activity/:index/:id' do
      ...
      app.post '/validate/:activity/:index' do
        load_user_and_activity
        answer = session["answer_for_#{params['activity']}_#{@index}"]
        ...
        if @test[:type] == :local_api
          ...
          if !opts
            ...
            answer = opts['error'] ? Samplers::UPLOAD_ERROR_MESSAGES[opts['message_index']] : opts['id'].to_s
          ...
          begin
            ...
            res[:answer] = CGI.escape_html(answer.to_s)
